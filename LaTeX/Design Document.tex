...
\section{RTL}
	...
	\subsection{Summary Chart}
		\begin{center} \begin{tabular}{| c | c | c | c | c | c | c | c |} \hline
			...
			\thead{Decode ins-\\truction and \\ get stuff from \\ registers} & \multicolumn{7}{c|}{\thead{R0 = reg\#IR[8:11] \\ R1 = (IR[3:0]==0x1) ? \{NextInst; PC += 2\} : reg\#IR[7:4] \\ ALUout = PC + SE(IR[15:12]) \\ Hreg = IR[3:0]}} \\ \hline
			\thead{Do \\ computation} & \thead{ALUout \\= R0$\dagger$R1} & \thead{PCtemp=ALUout \\ ALUout=R0$\dagger$R1} & \multicolumn{2}{c|}{\thead{ALUout = \\ R1 + SE(Hreg$<<$1)}} & \thead{SchLatch\\= Hreg} & \thead{Change\\on\\Code\#} & \thead{PC = R0+\\SE(IR[7:0]$<<$1)} \\ \hline
			\thead{Output} & \thead{@R0 = \\ALUout} & \thead{if(ALUout==0)\\PC = PCtemp} & \thead{MemRead = \\ MEM[ALUout]} & \thead{MEM[ALUout] \\= R0} & \multicolumn{3}{c|}{ } \\ \hline
			\thead{Output 2} & \multicolumn{2}{c|}{ } & \thead{@R0 = \\ MemRead} & \multicolumn{4}{c|}{ } \\ \hline
		\end{tabular} \end{center}
		Notations:
		\begin{itemize}
			\item[reg\#*:] The register at ID * in the register file
			\item[@*:] The original register (in the register file) that was put in *
			\item[*$\dagger$*:] Does what ever the ALU op says should be done to the 2 values
			\item[Code\#:] Varies based on on the sudo code number is
		\end{itemize}
	\subsection{Tests}
		\subsubsection{A-Type}
