...
\section{RTL}
	\subsection{Components}
		List of the components used in the the RTL in approximately the same order as they appear:
		\begin{itemize}
			\item[IR:]
			\item[MEM:]
			\item[PC:]
	...
	\subsection{Summary Chart}
		\begin{center} \begin{tabular}{| c | c | c | c | c | c | c | c |} \hline
			...
			\thead{Get \\ instruction} & \multicolumn{7}{c|}{\thead{IR = MEM[PC] \\ PC += 2}} \\ \hline
			\thead{Decode ins-\\truction and \\ get stuff from \\ registers} & \multicolumn{7}{c|}{\thead{if(OP==0x1)\{Imm = MEM[PC]; PC += 2\}\\ R0 = reg\#IR[8:11] \\ R1 = reg\#IR[7:4] \\ ALUout = PC + SE(IR[3:0]) \\ Hreg = IR[3:0]}} \\ \hline
			\thead{Do \\ computation} & \thead{ALUout = \\ R0 @ R1} & \thead{if(R0==R1):\\PC = ALUout} & \multicolumn{2}{c|}{\thead{ALUout = \\ R1 + SE(Hreg$<<$1)}} & \thead{SchLatch\\= Hreg} & \thead{Change\\on Code\#} & \thead{PC = R0\\+ IR[7:0]} \\ \hline
			\thead{Output} & \thead{R0 = \\ALUout} & & \thead{MemRead = \\ MEM[ALUout]} & \thead{MEM[ALUout] \\= R0} & \multicolumn{3}{c|}{ } \\ \hline
