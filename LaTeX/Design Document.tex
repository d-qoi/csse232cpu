	...
	\subsection{Summary Charts}
		\subsubsection{RTL}
			\begin{center} \begin{tabular}{| c | c | c | c | c | c | c | c |} \hline
				...
				\thead{Decode ins-\\truction and \\ get stuff from \\ registers} & \multicolumn{7}{c|}{\thead{R0 ::= reg\#IR[11:8] \\ if(IR[3:0]==0x1) \{R1 ::= NextInst; PC ::= PC + 2\} else \{R1 ::= reg\#IR[7:4]\} \\ ALUout ::= PC + SE(IR[3:0]) \\ Hreg ::= IR[3:0]}} \\ \hline
	...
	\subsection{Integration Tests}
		\subsubsection{Get instruction}
			\begin{tabular}{ r  p{15cm} }
				...
				2. & Toggle memRead and the instruction at the given address, as well as the next instruction, should now be in IR and NextInst respectively\\
